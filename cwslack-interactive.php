...
$requestobj = json_decode($_REQUEST['payload'], true);
//print_r($requestobj);
//file_put_contents("request.txt", print_r($_REQUEST), FILE_APPEND | FILE_USE_INCLUDE_PATH);

if(empty($requestobj['token']) || ($requestobj['token'] != $slacktoken)) die("Slack token invalid."); //If Slack token is not correct, kill the connection. This allows only Slack to access the page for security purposes.
//if(empty($requestobj['text'])) die("No text provided."); //If there is no text added, kill the connection.
$exploded = explode("_",$requestobj['callback_id']); //Explode the callback id attached to the callback for use in variables.
...
//	{
//		print("DEBUG: Dumping cURL data:\n");
//		var_dump($_REQUEST); //Dump the raw data.
//		file_put_contents("request.txt", var_dump($_REQUEST), FILE_APPEND | FILE_USE_INCLUDE_PATH);
//	}
//Timeout Fix Block
//if($timeoutfix == true)
//{
...
$attachments = $requestobj['original_message']['attachments'][0];
$ticketpretext = $attachments['pretext'];
$tickettitle = $attachments['title'];
$tickettext = $attachments['text'];
$tickettext = $attachments['callback_id'];
$senduser = $requestobj['user']['name'];
$responseURL = $requestobj['response_url'];
$return =array(
	"parse" => "full",
		"response_type" => "in_channel",
		"attachments"=>array(array(
			"title" => $tickettitle,
			"pretext" => $ticketpretext,
				"text" => "Ticket pulled by <@". $senduser. ">",
				"mrkdwn_in" => array(
					"text",
					"pretext"
					)
